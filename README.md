...
🧭 待处理
input/sentences.txt：有 50 行待导入
今日备份：尚未备份
数据检查：建议每周运行一次 --check
建议：先处理 input，再开始今日推荐复习。
```

待处理提醒只负责展示，不会自动导入、自动备份或自动检查数据。
...
- 自评完成后，进入下一题前才清屏
- 第 1 题开始前会直接清屏
- 第 2 题及以后，会先显示逐行倒计时，再清屏进入下一题
- 错题毕业、移入 master、从 wrong 移除等提示会保留到倒计时结束
- 答错后的“再练一次”开始前也会显示逐行倒计时再清屏
...
正确度只是字面相似度参考，不代表语义或语法完全正确。最终是否掌握仍由你自评决定。

V16.4 起，Quiz 自评支持回车默认掌握。答题后出现：

```text
这题掌握了吗？回车/y=掌握，n=加入错题，q=退出：
```
...
- 直接回车：等同于 `y`，表示掌握
- `y`：掌握
- `n`：未掌握，普通 Quiz 中会加入 wrong 池
- `q`：退出当前 Quiz

这个规则只适用于自评环节，不影响 Quiz 答案输入、快速添加、菜单输入或 reset 确认。

V16.4.3 起，普通 Quiz 中如果某题正确度很高，并且你已经自评为掌握，程序会询问是否加入 master：

```text
这题正确度很高，是否标记为完全掌握并加入 mastered.md？回车/y=加入，n=不加入，q=退出：
```

此时：

- 直接回车：等同于 `y`，加入 master
- `y`：加入 master
- `n`：不加入 master，继续留在 review
- `q`：退出当前 Quiz

这个规则只适用于 master 确认环节，不影响 Quiz 答案输入、自评输入、菜单输入、快速添加或 reset 确认。
