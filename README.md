...
V11 版本保持简单：不联网，不调用 AI，不做自动纠错，只使用 Python 标准库。V10.1 优化了终端输出格式，V10.2 在保留美观输出的基础上加固了 macOS 和 Windows 11 的路径、编码、CSV 导出兼容性。
...
- 标签、语法点、重点单词都是可选字段。
- 如果没有填写标签、语法点或重点单词，程序会保存为空字符串，不会强制写入默认文本。
- 程序不会自动识别语法点或重点单词，需要你手动填写，或让 ChatGPT 生成后再粘贴导入。
- 空行会被忽略。
...
它不会写入 `input/sentences.txt`，也不会触发清空输入文件提示。`--tag`、`--grammar`、`--words` 都是可选字段，未填写时保存为空字符串。
...
目前不会自动判对错，因为日语表达可能有多种，需要你自己对照参考答案自评。
...
生成的复习本、每日归档、错题本和已掌握本都会保存标签、语法点、重点单词字段。
