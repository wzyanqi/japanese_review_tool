...
V15.3 起，菜单顶部会显示“今日推荐”，根据 review / wrong / master 三池状态和今日新增数量，推荐普通 Quiz、错题 Quiz 或新增句子。选择“开始今日推荐复习”后，程序会直接进入推荐的复习流程；如果当前没有可复习内容，会提示使用快速添加或批量导入。

普通 Quiz 和错题 Quiz 都有子菜单，可以选择随机抽题、指定数量或 loop 模式。快速添加会依次询问日语、中文、标签、语法点、重点单词和备注，并在确认后加入 review 池。导入和备份会先询问确认。

V15.1 起，不再单独提供“完全掌握确认”菜单入口。普通 Quiz 中，如果你的输入正确度很高，并且自评为掌握，程序会询问是否将该句标记为完全掌握并移入 `mastered.md`。

菜单中也可以选择“今天该做什么”，它会根据当前三池状态和今日新增数量给出下一步建议。

菜单中也可以选择“最近有没有坚持”，用 Console 字符条形图查看最近几天的学习活动。

在菜单中可以输入数字选择功能，输入 `0`、`q` 或 `quit` 返回或退出。执行完一个功能后，按回车会返回主菜单。

V14.6 起，菜单模式支持日文输入法下的全角数字，例如 `２` 会识别为 `2`，`１０` 会识别为 `10`，`０` 会识别为 `0`。V15.2 起，菜单数字输入也支持 `１ ０` / `１　０` 这样的空格分隔写法，并识别为 `10`。全角 `ｑ` / `Ｑ` 也可以作为退出输入，因此使用日文输入法时不需要特意切回英文输入法来选择菜单。
...
从 V13.3 开始，`--stats` 和 `--today` 会按三池模型展示当前学习状态，明确区分 review、wrong、master，不再把 `japanese_review.md` 当作旧意义上的总复习本。
...
Quiz 显示参考答案后，程序会自动显示“差异提示”，帮助你快速看出你的输入和参考答案哪里不一样。
...
差异提示只用于辅助学习，不影响正确度算法，也不会自动改写你的答案。纯标点或空格差异会尽量过滤，避免输出太吵。
...
Quiz 结束时会显示“本轮复习小结”，包括本轮抽查题数、新增错题列表、进入 master 的句子、重答次数和下一步建议。小结只显示在 Console，不会写入文件，也不会改变三池流转逻辑。

V15.4 起，每次普通 Quiz 或错题 Quiz 结束后，程序会在本轮复习小结之后追加显示长期进度反馈，包括今日进度、本周进度、累计掌握和下一步建议。
...
如果句子总数少于 `--count`，程序会允许重复抽取，适合反复强化记忆。V13.7 起，Quiz 抽题会尽量避免连续两题出现同一句；如果当前候选题只有 1 条，则仍可能重复出现。
...
统计面板还会显示三池合计的标签总数，以及句子最多的前 5 个标签。

## 今日复习建议

`--plan` 用来回答“我接下来应该做什么”。它会根据当前三池状态和今日新增情况，给出最多 3 条简单建议和推荐命令。
...
```powershell
python japanese_review.py --plan
```

`--plan` 只读取这些数据，不会修改任何文件：
